...
## Study Abstract:
Skin cancer diagnosis using computer vision has advanced substantially, yet significant performance disparities across skin tones persist, with models often underperforming on darker skin due to biased pretrained representations and dataset imbalance. This study investigates Low-Rank Adaptation (LoRA) as a parameter-efficient fine-tuning strategy for mitigating skin tone–related bias in Vision Transformer–based skin cancer detection models. Using the Diverse Dermatology Images (DDI) dataset, we evaluate multiple DINOv2 backbone variants across different LoRA ranks, analysing overall performance, subgroup fairness metrics, and computational efficiency specifically between Fitzpatrick I–II and V–VI groups. Linear probing reveals pronounced inherent algorithmic bias in pretrained models, with the DINOv2-Base linear probe exhibiting an AUC gap of 0.20 and a recall gap of 0.24 between Fitzpatrick I–II and V–VI groups. In contrast, LoRA fine-tuning with rank 8 reduces the AUC gap to 0.01 and the recall gap to 0.05 while improving overall AUC from 0.70 to 0.76 and recall from 0.74 to 0.80. These gains are achieved with only 2.4 MB of additional trainable parameters and without increasing inference latency. Grad-CAM analysis further supports interpretability by revealing lesion-focused attention in high-confidence predictions alongside occasional artefact reliance. Overall, the results demonstrate that Low Rank Adaptation offers a computationally efficient and scalable pathway toward fairness-aware skin lesion analysis.
